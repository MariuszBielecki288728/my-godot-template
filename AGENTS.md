...
Gameplay/project code belongs outside `tools/`; `tools/` is the isolated Python development
subsystem. `dependencies.json` is the source of truth for manifest-managed Godot/project
dependencies, while `tools/pyproject.toml` plus `tools/uv.lock` define Python tooling.
...
reason. Never silently upgrade Godot, GUT, or core tooling. Third-party dependencies in
`dependencies.json` are generated by bootstrap: do not edit or commit generated directories
such as `addons/gut/`. Update a dependency's version, URL, and verified SHA-256 together.

Do not edit `uv.lock` manually. Changes to Python tooling require Ruff and Pytest validation.
